"""Build NLSTseg manifest rows and append to composite manifest."""
...
DEFAULT_NLSTSEG_ROOT = DEFAULT_RAW_DIR / "nlstseg"
DEFAULT_CASES_DIR = DEFAULT_NLSTSEG_ROOT / "cases"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build NLSTseg manifest rows for composite segmentation training.")
    parser.add_argument("--nlstseg-root", type=Path, default=DEFAULT_NLSTSEG_ROOT)
    ...
    parser.add_argument("--replace-nlstseg-rows", action="store_true")
...
def load_pairs_from_csv(pairs_csv: Path, root_dir: Path) -> list[PairSpec]:
    ...
    with pairs_csv.open("r", encoding="utf-8", newline="") as handle:
        ...
        for index, row in enumerate(reader, start=1):
            ...
            case_id = str((row.get("case_id") or "").strip()) or f"nlstseg_{index:05d}"
            pairs.append(
                PairSpec(
...
def discover_pairs(root_dir: Path, max_cases: int) -> list[PairSpec]:
    ...
    for mask_path in masks:
        ...
        case_id = sanitize(f"nlstseg_{strip_mask_suffix(relative.as_posix())}")
...
def load_volume(path: Path, preferred_keys: tuple[str, ...], is_mask: bool) -> tuple[np.ndarray, tuple[float, float, float]]:
    ...
    try:
        ...
        raise ModuleNotFoundError(
            "SimpleITK is required for NLSTseg ingest when using medical volume files. Install with `pip install SimpleITK`."
        ) from exc
...
def merge_rows(existing: list[dict[str, str]], incoming: list[dict[str, str]], replace_nlstseg_rows: bool) -> list[dict[str, str]]:
    ...
    for row in existing:
        ...
        if not case_id:
            continue
        if replace_nlstseg_rows and source == "nlstseg":
            continue
        merged[case_id] = row
    for row in incoming:
...
def main() -> None:
    ...
    if args.pairs_csv is not None:
        pairs = load_pairs_from_csv(args.pairs_csv, args.nlstseg_root)
    else:
        pairs = discover_pairs(args.nlstseg_root, max_cases=max(int(args.max_cases), 0))
    ...
    merged = merge_rows(existing, rows, replace_nlstseg_rows=bool(args.replace_nlstseg_rows))
    write_manifest(args.manifest_path, merged)
    print(f"Wrote manifest: {args.manifest_path}")
    print(f"added_nlstseg_rows={len(rows)} total_rows={len(merged)}")
