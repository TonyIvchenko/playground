...
def test_split_and_train_model_runs(tmp_path: Path):
    ...
    nodule_weight = np.array([1.0, 1.2] * 10, dtype=np.float32)
    malignancy_weight = np.array([0.0, 1.0] * 10, dtype=np.float32)
    ...
    x, y_nodule, y_malignancy, y_malignancy_mask, y_nodule_weight, y_malignancy_weight, loaded_series_ids = load_training_dataset(dataset_path)
    split = split_dataset(
        x,
        y_nodule,
        y_malignancy,
    ...
    with torch.no_grad():
        output = result.model((split[6] - result.patch_mean) / result.patch_std)
