"""Train wildfires ignition PyTorch model from real wildfires records."""
...
def load_raw_dataset(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(
            f"Dataset not found at {path}. Run: `python -m src.disasters.scripts.wildfires_download_data`"
        )
...
def train_model(
    ...
    weight_decay: float,
    seed: int,
) -> tuple[WildfiresMLP, torch.Tensor, torch.Tensor, float, float, float]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train wildfires PyTorch model from real data.")
    parser.add_argument(
        "--input-csv",
        type=Path,
