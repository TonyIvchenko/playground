"""Hurricane service with Gradio UI + Google Maps monthly tile overlays."""
...
def _load_overlay() -> tuple[np.ndarray, np.ndarray, list[str], dict[str, float | int | str]]:
    ...
    if CUBE_PATH.exists():
        ...
        risk = np.zeros((len(frames), 160, 220), dtype=np.float32)
...
def _map_html() -> str:
    ...
    return f"""
<div id="hurricane-map-shell" class="risk-map-shell">
  <div class="risk-map-header">
    <div class="control-row">
      <label for="hurricane-time-slider"><strong>Time:</strong> <span id="hurricane-time-value">{FRAMES[0]}</span></label>
      <button id="hurricane-play" type="button">Play</button>
      <select id="hurricane-layer">
        <option value="risk">Risk Probability</option>
        <option value="activity">Activity Intensity</option>
        <option value="confidence">Confidence</option>
      </select>
      <span class="risk-phase">Section: <span id="hurricane-phase">training</span></span>
    </div>
    <input id="hurricane-time-slider" type="range" min="0" max="{frame_count - 1}" value="0" step="1" />
  </div>
  ...
  <div id="hurricane-map" class="risk-map"></div>
  <div id="hurricane-map-status" class="risk-map-status"></div>
...
<script>
(() => {{
  const root = document.getElementById("hurricane-map-shell");
  if (!root || root.dataset.ready === "1") return;
  root.dataset.ready = "1";

  const cfg = {json.dumps(js_config)};
  const slider = root.querySelector("#hurricane-time-slider");
  const valueNode = root.querySelector("#hurricane-time-value");
  const phaseNode = root.querySelector("#hurricane-phase");
  const layerSelect = root.querySelector("#hurricane-layer");
  const playBtn = root.querySelector("#hurricane-play");
  const mapNode = root.querySelector("#hurricane-map");
  const statusNode = root.querySelector("#hurricane-map-status");
