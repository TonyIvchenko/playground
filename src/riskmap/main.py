...
from src.hurricane.model import load_model_bundle as load_hurricane_model_bundle
from src.wildfire.model import load_model_bundle as load_wildfire_model_bundle
...
WILDFIRE_MODEL_PATH = Path(
    os.getenv("WILDFIRE_MODEL_PATH", str(Path(__file__).resolve().parents[1] / "wildfire" / "model" / "wildfire_model.pt"))
)
HURRICANE_MODEL_PATH = Path(
    os.getenv("HURRICANE_MODEL_PATH", str(Path(__file__).resolve().parents[1] / "hurricane" / "model" / "hurricane_model.pt"))
)

WILDFIRE_TILES_DIR = Path(__file__).resolve().parents[1] / "wildfire" / "tiles"
HURRICANE_TILES_DIR = Path(__file__).resolve().parents[1] / "hurricane" / "tiles"
