...
To verify the motion-heavy browser apps still honor reduced-motion preferences, run `python scripts/check_browser_app_reduced_motion.py`.
To verify the browser apps still use the shared footer contract for caveats, privacy notes, and local-only disclaimers, run `python scripts/check_browser_app_footers.py`.
To verify the browser apps still use the shared typography contract instead of local font imports or one-off font stacks, run `python scripts/check_browser_app_typography.py`.
To verify the browser apps still expose the shared help and shortcuts drawer, run `python scripts/check_browser_app_help_drawers.py`.
To verify the browser apps keep their audited text-contrast pairs above the floor, run `python scripts/check_browser_app_contrast.py`.
To verify the browser apps still load the shared starter shell and surface stylesheet, run `python scripts/check_browser_app_starter_css.py`.
To verify the browser apps keep shared status wording for loading, ready, fallback, and failure states, run `python scripts/check_browser_app_status_copy.py`.
...
The first pass intentionally stays small and green: root helper scripts, README markdown consistency, docs spelling, Docker smoke doc checks for containerized services, tracked JSON/YAML config linting, tracked junk-file checks, browser-app header, empty-state, loading-state, fallback-badge, and error-state contract checks, type-specific service file checks, the shared browser token server/helper paths, service `main.py` entrypoints, the static browser-app smoke suite, and the health/app smoke tests we already maintain for `ctscan`, `disasters`, and `voiceforge`.
